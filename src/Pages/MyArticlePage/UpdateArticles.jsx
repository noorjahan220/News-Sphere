...
const UpdateArticles = () => {
    ...
    useEffect(() => {
        if (data) {
            ...
            const tagsArray = Array.isArray(data.tags) ? data.tags : data.tags.split(',').map(tag => tag.trim());
            setValue('tags', tagsArray.join(', ')); // For displaying as a string in the input field
    ...
    const onSubmit = async (formData) => {
        // Check if there are no changes before submitting
        if (
            ...
            formData.image === articleData.image // Also compare the image
        ) {
        ...
        try {
            ...
            if (formData.image && formData.image[0]) {
                // Upload the image to the hosting service first (using your image hosting API)
                ...
                imageUrl = imageResponse.data.data.url; // Get the URL of the uploaded image
            }

            updatedFormData.append('image', imageUrl); // Send the image URL instead of the file itself
            ...
            refetch(); // refetch the data to get updated content
